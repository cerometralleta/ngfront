...
import { DataViewModule, Button, TreeOptions, DataFilter } from '../metadata/dataViewModule.md';
...
import { DataViewResolver } from '../resolver/dataViewResolver';
...
export class DataViewEditComponent implements OnInit {
  ...
  createColumnGroup(columnOptions) {
    return this.fb.group({
      ...
      unduplicated: [columnOptions.unduplicated],
 ...
 showTreeCheck() {
  ...
  clearDatafilter() {
    const controls = <FormArray>this.ngbForm.controls['dataFilters'];
    for (let idx = controls.length; idx >= 0; idx--) {
      controls.removeAt(idx);
    }
  }
